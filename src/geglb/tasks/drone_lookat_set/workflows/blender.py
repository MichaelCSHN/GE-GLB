"""Blender capture workflow for Task 03 Drone LookAt View Set.

Consumes a ``DroneLookAtSpec`` and vehicle poses, then produces a complete
``ge-glb.dataset/v1`` dataset plus a ``blender-job.json`` that is
consumable by ``scripts/blender_capture.py``.
"""
...
def build_blender_plan(
    ...
    """Prepare a deterministic Blender render job for a drone LookAt view set.
    ...
    render_engine :
        Blender render engine identifier.
    samples :
...
def _write_lookat_dataset(
    ...
    """Write a ``ge-glb.dataset/v1`` tree for a drone LookAt capture model.

    Adapted from the Task 02 pattern; omits ``fusion_plan``.
    """
    output.mkdir(parents=True, exist_ok=True)
